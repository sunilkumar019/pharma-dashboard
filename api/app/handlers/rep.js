...
const repLogin = async(req, res, next) => {
    let { phone, otp, device_token } = req.body;

    if (!phone) return next(new Error("Please Provide email or phone"))
    ...
    try {
        let rep = await repController.repLogin({ phone, otp, device_token })
        if (rep.Error) {
            req.data = null;
            req.status = 403;
...
const generateOtp = async (req, res, next) => {
    try {
        const rs = await repController.generateOtp(req.body);
        req.data = null;
        req.message = rs;
        next();
    } catch (e) {

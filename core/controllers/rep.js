...
const otpHandler = require('../usecases/rep/otp');
...
exports.repLogin = async (repprops) => {
    let repRecord = await getRep({ phone: repprops.phone });   

    if (repRecord.length == 0)
        return { Error: "Invalid Credentials" }

    const isOtpMatched = await otpHandler.validateOTP( {otp: repprops.otp, hash:repRecord[0].otp});
    if (!isOtpMatched.status)
...
exports.generateOtp = async (body) => {
    ...
    const repRecord = await getRep({ phone: body.phone });
    if(!repRecord.length) {
        throw new Error("No Data found with this number.")
    }

    // Generating OTP
    const otp = Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000;

    const { status } = await otpHandler.hashingOTP({ otp, repId: repRecord[0]._id });

    // Bind api for sending OTP to phone
    const otpSendApi = await axios.get(`${process.env.SMS_URL}&number=${body.phone}&text=${otp} is your One Time Password for request placed on PharmaHopers. Thanks!`)

    // After send otp successful
    return status && otpSendApi.status === 200 ? 'OTP sent successfully' : 'Something happened wrong!';
}
